...
**关系**: 
- `has_many :api_tokens` → API token 资源
...
**关系**: 
- `belongs_to :ash_user` → 用户（`ash_users`）
...
**模块**: Vmemo.Account.ApiToken（Ash 资源；对应模块在生成的 API 文档中隐藏）
...
**服务模块**: Vmemo.SearchEngine.TsNote（内部模块；在生成的 API 文档中隐藏）
...
```
User (1) ──────┬────── (N) Session   [会话 / ash_user_tokens]
               │
               └────── (N) API token [API 密钥 / api_tokens]
```
...
| 功能领域 | PostgreSQL 表 | 应用模块 | Typesense 集合 | 服务模块 |
|---------|--------------|----------|---------------|---------|
