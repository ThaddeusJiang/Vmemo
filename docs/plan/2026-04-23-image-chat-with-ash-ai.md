...
目标是在现有图片浏览体验中增加右下角 `Ask AI` 入口，用户可在右侧 Drawer 与 AI 围绕当前图片交流；同时保留全局聊天能力，并明确会话模型、命令行为、模型路由与验收标准。
...
- 同一用户在同一图片详情页只能有一个 `image_scoped` 会话（重复进入应复用已有会话）。
- 从 `/chat` 可创建任意多个 `global` 会话，且 `image_id` 为空。

### 2.2 UI/交互

- 图片详情右下角展示 `Ask AI` 按钮。
- 点击后在右侧弹出 Drawer 聊天面板。
- 聊天面板需支持消息中显示图片内容。
...
关键约束：
- 条件唯一约束：同一 `(user_id, image_id)` 下仅允许一个 `kind=image_scoped` 会话。
- `kind=global` 不受上述唯一约束限制。
...
- 提供统一入口 `get_or_create_image_scoped_conversation(user, image)`：
  - 先查，后建，保证幂等。
  - 并发下由数据库唯一约束兜底。
...
- 右下角增加 `Ask AI` 按钮。
- 点击后打开右侧 Drawer。
- Drawer 首次打开时加载或创建 `image_scoped` 会话。
- 明确展示“当前图片上下文”标识，避免用户混淆。
...
- 完成会话类型与约束落地（`image_scoped` / `global`）。
- 图片详情页 Ask AI + Drawer。
- 基础消息收发与图片消息显示。
- `/compact`、`/clear` 基础可用。
...
1. 图片详情唯一会话
- Given：用户在同一图片详情页多次点击 Ask AI
- When：系统尝试创建会话
- Then：始终进入同一个 `image_scoped` 会话
...
2. 并发正确性
- 并发触发 image chat 创建时，不出现重复 `image_scoped` 会话
