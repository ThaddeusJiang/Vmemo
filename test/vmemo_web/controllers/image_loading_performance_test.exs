defmodule VmemoWeb.ImageLoadingPerformanceTest do
  ...
  test "warm list and detail responsive image variants respond within one second", %{
    conn: conn,
    user: user,
    image_dir: image_dir
  } do
    ...
    list_path = ~p"/storage/v1/#{user.id}/images/performance--160w.webp"
    detail_path = ~p"/storage/v1/#{user.id}/images/performance--640w.webp"

    assert_request_under(conn, list_path, @threshold_ms)
