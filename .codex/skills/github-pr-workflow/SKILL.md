...
为避免遗漏基础检查，执行 PR 流程时必须遵循以下闸门：

1. 在 `git commit` 前，必须先实际执行（不是口头确认）：
   - `mix format`
   - `mix compile --warnings-as-errors`
   - `mix test`
2. 在 `git push` 前，若有任意新增改动/新增提交，必须重新执行上述三项（至少受影响项；默认全跑）。
3. 在 `gh pr create` 或 `gh pr edit` 前，必须再次确认“当前工作区对应提交”已通过检查；未通过禁止继续。
4. 任何一步失败都必须先修复再继续，禁止“先开 PR 让 CI 兜底”。
5. 执行 PR create/update 时，在最终输出中必须附一段“本地检查结果摘要”（执行了哪些命令、是否通过）。
